...
////////////////////////////////////////////
///   PARTICLE BUILDER   ///////////////////    //TODO: CLEAN THIS UP A TON - LOOK AT UNIT TEST FOR INSTRUCTIONS
////////////////////////////////////////////
var ParticleBuilder = {
    ...
    _create_particle_body : function(particle, particle_data) {
        ...
        if (particle_data.image) {
            ...
            particle.body = ParticleBuilder.TYPES.RECTANGLE;
    ...
    create_particle : function(particle, particle_data) {
        ...
        particle.time_total = particle_data.lifetime_mseconds +
                                Nickel.UTILITY.random_number(particle_data.lifetime_variation_vector_mseconds[0],
                                                             particle_data.lifetime_variation_vector_mseconds[1]);
        ...
        particle.scaleg = [1,1];
        particle.scalev = [[Nickel.UTILITY.round(particle_data.scale_mult_variation_vector[0][0], 6),
                            Nickel.UTILITY.round(particle_data.scale_mult_variation_vector[0][1], 6)],
                           [Nickel.UTILITY.round(particle_data.scale_mult_variation_vector[1][0], 6),
                            Nickel.UTILITY.round(particle_data.scale_mult_variation_vector[1][1], 6)]];
        
        particle.rot  = particle_data.rotation_degrees * Math.PI / 180;
        particle.rotv = [Nickel.UTILITY.round(
                            particle_data.rotation_variation_degrees_vector[0] * Math.PI / 180, 6),
                         Nickel.UTILITY.round(
                            particle_data.rotation_variation_degrees_vector[1] * Math.PI / 180, 6)];
        particle.rot = particle.rot + Nickel.UTILITY.random_number(particle.rotv[0],
                                                                   particle.rotv[1]);
        particle.vel  = particle_data.velocity_vector;
        particle.velv = [[Nickel.UTILITY.round(particle_data.velocity_variation_vector[0][0], 6),
                          Nickel.UTILITY.round(particle_data.velocity_variation_vector[0][1], 6)],
                         [Nickel.UTILITY.round(particle_data.velocity_variation_vector[1][0], 6),
                          Nickel.UTILITY.round(particle_data.velocity_variation_vector[1][1], 6)]];
        
        particle.acc  = particle_data.acceleration_vector;
        particle.accv = [[Nickel.UTILITY.round(particle_data.acceleration_variation_vector[0][0], 6),
                          Nickel.UTILITY.round(particle_data.acceleration_variation_vector[0][1], 6)],
                         [Nickel.UTILITY.round(particle_data.acceleration_variation_vector[1][0], 6),
                          Nickel.UTILITY.round(particle_data.acceleration_variation_vector[1][1], 6)]];
        
        // ceate body of particle
        ParticleBuilder._create_particle_body(particle, particle_data);
        ...
        if (particle_data.image) {
            particle.draw = function() {
                ...
                ctx.scale(particle.scale[0] * particle.scaleg[0],
                          particle.scale[1] * particle.scaleg[1]);
            ...
            switch (particle.body) {
                case ParticleBuilder.TYPES.RECTANGLE:
                    particle.draw = function() {
                        var ctx = particle.context;
                ...
                case ParticleBuilder.TYPES.ELLIPSE:
                    particle.draw = function() {
                        var ctx = particle.context;
                        ctx.save();
                        ctx.translate(particle.pos[0] + particle.size[0]/2,
        ...
        particle.move = function() {
            ...
            // scale
            particle.scale = [particle.scale[0] * Nickel.UTILITY.random_number(particle.scalev[0][0]*1000000,
                                                                    particle.scalev[0][1]*1000000) / 1000000,
                              particle.scale[1] * Nickel.UTILITY.random_number(particle.scalev[1][0]*1000000,
                                                                    particle.scalev[1][1]*1000000) / 1000000];
            ...
            // acceleration
            var tmp_acc  = [particle.acc[0] + Nickel.UTILITY.random_number(particle.accv[0][0]*1000000,
                                                                particle.accv[0][1]*1000000) / 1000000,
                            particle.acc[1] + Nickel.UTILITY.random_number(particle.accv[1][0]*1000000,
                                                                particle.accv[1][1]*1000000) / 1000000];
            
            // veocity
            particle.vel = Nickel.UTILITY.add_vector(particle.vel, tmp_acc);
            var tmp_vel  = [particle.vel[0] + Nickel.UTILITY.random_number(particle.velv[0][0]*1000000,
                                                                particle.velv[0][1]*1000000) / 1000000,
                            particle.vel[1] + Nickel.UTILITY.random_number(particle.velv[1][0]*1000000,
                                                                particle.velv[1][1]*1000000) / 1000000];
...
////////////////////////////////////////////
///   PARTICLE SYSTEM BUILDER   ////////////    //TODO: CLEAN THIS UP A TON - LOOK AT UNITTEST FOR INSTRUCTIONS
////////////////////////////////////////////
var ParticleSystemBuilder = {
    ...
    create_particle_system_base : function(sys, sys_data) {
        ...
        sys.period = sys_data.create_period_mseconds;
        sys.amount = sys_data.create_amount;
        ...
        sys.update = function() {
            ...
            if (sys.period <= now - sys.last_created) {
                sys.last_created = now;
                for (var i=0; i<sys.amount; i++) {
                    var particle = new Particle(sys.particle_data);
                    sys.apply_init_features(particle);
                    particle.pos = [particle.pos[0] + sys.pos[0],
                                    particle.pos[1] + sys.pos[1]];
                    sys.queue.in(particle);
                }
            }
            
            // remove dead particles
            while (sys.queue.next().dead)
                sys.queue.out();
            ...
            var ctx = sys.context;
            ctx.save();
            ctx.translate(sys.pos[0], sys.pos[1]); ///////////////////////////////////////////////////////// HERE
            ctx.rotate(sys.rot);
...
////////////////////////////////////////////
///   PARTICLE BUILDER   ///////////////////    // (REDOING ABOVE)
////////////////////////////////////////////
var ParticleBuilder = {
    ...
    create_base : function(particle, system, data) {
        ...
        particle.init = function() {
            system.apply_init_features(particle);
        }
        ...
        particle.draw = function() {
            ...
            system.apply_draw_features(particle);
        ...
        // - moves image of body
        particle.move = function() {
            system.apply_move_features(particle);
        }
    ...
    create_features : function(ptc, system, data) {
        ...
        var _round_matrix_2x2 = function(matrix_2x2){
            return [[Nickel.UTILITY.round(matrix_2x2[0][0], 6),
                    Nickel.UTILITY.round(matrix_2x2[0][1], 6)],
                    [Nickel.UTILITY.round(matrix_2x2[1][0], 6),
                    Nickel.UTILITY.round(matrix_2x2[1][1], 6)]];
        }

        var _round_rgba = function(rgba){
            return [[Nickel.UTILITY.round(rgba[0][0], 6),
                    Nickel.UTILITY.round(rgba[0][1], 6)],
                    [Nickel.UTILITY.round(rgba[1][0], 6),
                    Nickel.UTILITY.round(rgba[1][1], 6)],
                    [Nickel.UTILITY.round(rgba[2][0], 6),
        ...
        var _rand_num = function(vec_2){
            return Nickel.UTILITY.random_number(vec_2[0]*1000000,
                                                vec_2[1]*1000000) / 1000000;
        }
        ...
        // convert any input degrees to radians

        if (data.start_rotation != null)
             data.start_rotation *= Math.PI / 180;

        if (data.start_rotation_variation)
            data.start_rotation_variation = [
                data.start_rotation_variation[0] * Math.PI / 180,
        ...
        if (data.lifetime)
            system.add_init_feature(function(particle) {
                particle.time_total = data.lifetime;
            });
        else
            system.add_init_feature(function(particle) {
        ...
        // lifetime variation
        if (data.lifetime_variation) {
            var lifev = _round_vec_2(data.lifetime_variation);
            system.add_init_feature(function(particle) {
                particle.time_total += _rand_num(lifev);
            });
        }
        ...
        if (data.start_position)
            system.add_init_feature(function(particle) {
                particle.pos = [data.start_position[0],
        ...
        if (data.start_rotation != null)
            system.add_init_feature(function(particle) {
                particle.rot = data.start_rotation;
            });
        else
            system.add_init_feature(function(particle) {
        ...
        // start rot variation
        if (data.start_rotation_variation) {
            var rotv = _round_vec_2(data.start_rotation_variation);
            system.add_init_feature(function(particle) {
                particle.rot += _rand_num(rotv);
            });
        }
        ...
        if (data.start_scale)
            system.add_init_feature(function(particle) {
                particle.scale = [data.start_scale[0],
                                  data.start_scale[1]];
            });
        else
            system.add_init_feature(function(particle) {
        ...
        // start opacity
        if (data.start_opacity != null)
            system.add_init_feature(function(particle) {
                particle.op = data.start_opacity;
            });
        
        // start opacity variation
        if (data.start_opacity_variation) {
            var opv = _round_vec_2(data.start_opacity_variation);
            system.add_init_feature(function(particle) {
                particle.op += _rand_num(opv);
            });
        }
        ...
        if (data.scale_acceleration)
            system.add_init_feature(function(particle) {
                particle.scaleacc = [data.scale_acceleration[0],
        ...
        if (data.scale_acceleration)
            system.add_move_feature(function(particle) {
                particle.scalevel[0] *= particle.scaleacc[0];
                particle.scalevel[1] *= particle.scaleacc[1];
            });
        
        // opacity acceleration (init)
        if (data.opacity_acceleration != null)
            system.add_init_feature(function(particle) {
        ...
        if (data.opacity_acceleration_variation) {
            var opaccv = _round_vec_2(data.opacity_acceleration_variation);
            system.add_move_feature(function(particle) {
                particle.opvel += _rand_num(opaccv);
            });
        }
        
        // opacity acceleration (move)
        if (data.opacity_acceleration != null)
            system.add_move_feature(function(particle) {
                particle.opvel += particle.opacc;
            });
        ...
        if (data.opacity_velocity_variation) {
            var opvelv = _round_vec_2(data.opacity_velocity_variation);
            system.add_move_feature(function(particle) {
                particle.op += _rand_num(opvelv);
            });
        }
        ...
        var ctx = ptc.context;
        if (ptc.body == ParticleBuilder.TYPES.ELLIPSE) {
        ...
        } else if (ptc.body == ParticleBuilder.TYPES.RECTANGLE) {
            if (data.enable_translation) {
                if (data.enable_rotation) {
                    system.add_draw_feature(function(particle){
                        ctx.translate(particle.pos[0],
        ...
        if (data.type != ParticleBuilder.TYPES.IMAGE) {
            ...
            if (data.fill_color)
                system.add_init_feature(function(particle) {
                    particle.fill = [data.fill_color[0],
                                     data.fill_color[1],
                                     data.fill_color[2],
                                     data.fill_color[3]];
                });
            ...
            if (data.fill_color_variation) {
                var fillv = _round_rgba(data.fill_color_variation);
                system.add_init_feature(function(particle) {
                    particle.fill[0] += _rand_num(fillv[0]);
                    particle.fill[1] += _rand_num(fillv[1]);
                    particle.fill[2] += _rand_num(fillv[2]);
                    particle.fill[3] += _rand_num(fillv[3]);
                });
            }

            // fill acceleration (init)
            if (data.fill_acceleration)
                system.add_init_feature(function(particle) {
                    particle.fillacc = [data.fill_acceleration[0],
                                        data.fill_acceleration[1],
                                        data.fill_acceleration[2],
                                        data.fill_acceleration[3]];
                });

            // fill acceleration variation
            if (data.fill_acceleration_variation) {
                var fillaccv = _round_rgba(data.fill_acceleration_variation);
                system.add_move_feature(function(particle) {
                    particle.fillvel[0] += _rand_num(fillaccv[0]);
                    particle.fillvel[1] += _rand_num(fillaccv[1]);
                    particle.fillvel[2] += _rand_num(fillaccv[2]);
                    particle.fillvel[3] += _rand_num(fillaccv[3]);
                });
            }

            // fill acceleration (move)
            if (data.fill_acceleration)
                system.add_move_feature(function(particle) {
                    particle.fillvel[0] += particle.fillacc[0];
                    particle.fillvel[1] += particle.fillacc[1];
                    particle.fillvel[2] += particle.fillacc[2];
                    particle.fillvel[3] += particle.fillacc[3];
                });
            
            // fill velocity (init)
            if (data.fill_velocity)
                system.add_init_feature(function(particle) {
                    particle.fillvel = [data.fill_velocity[0],
                                        data.fill_velocity[1],
            ...
            if (data.fill_velocity_variation) {
                var fillvelv = _round_rgba(data.fill_velocity_variation);
                system.add_move_feature(function(particle) {
                    particle.fill[0] += _rand_num(fillvelv[0]);
                    particle.fill[1] += _rand_num(fillvelv[1]);
                    particle.fill[2] += _rand_num(fillvelv[2]);
                    particle.fill[3] += _rand_num(fillvelv[3]);
                });
            }
            ...
            if (data.fill_velocity)
                system.add_move_feature(function(particle) {
                    ...
                    particle.fill[2] += particle.fillvel[2];
                    particle.fill[3] += particle.fillvel[3];
                });
            ...
            if (data.stroke_color)
                system.add_init_feature(function(particle) {
                    particle.color = [data.stroke_color[0],
                                      data.stroke_color[1],
                                      data.stroke_color[2],
                                      data.stroke_color[3]];
                });
            ...
            if (data.stroke_color_variation) {
                var colv = _round_rgba(data.stroke_color_variation);
                system.add_init_feature(function(particle) {
                    particle.color[0] += _rand_num(colv[0]);
                    particle.color[1] += _rand_num(colv[1]);
                    particle.color[2] += _rand_num(colv[2]);
                    particle.color[3] += _rand_num(colv[3]);
                });
            }
            
            // color acceleration (init)
            if (data.color_acceleration)
                system.add_init_feature(function(particle) {
                    particle.coloracc = [data.color_acceleration[0],
                                         data.color_acceleration[1],
                                         data.color_acceleration[2],
                                         data.color_acceleration[3]];
                });

            // color acceleration variation
            if (data.color_acceleration_variation) {
                var coloraccv = _round_rgba(data.color_acceleration_variation);
                system.add_move_feature(function(particle) {
                    particle.colorvel[0] += _rand_num(coloraccv[0]);
                    particle.colorvel[1] += _rand_num(coloraccv[1]);
                    particle.colorvel[2] += _rand_num(coloraccv[2]);
                    particle.colorvel[3] += _rand_num(coloraccv[3]);
                });
            }
            
            // color acceleration (move)
            if (data.color_acceleration)
                system.add_move_feature(function(particle) {
                    particle.colorvel[0] += particle.coloracc[0];
                    particle.colorvel[1] += particle.coloracc[1];
                    particle.colorvel[2] += particle.coloracc[2];
                    particle.colorvel[3] += particle.coloracc[3];
                });
            
            // color velocity (init)
            if (data.color_velocity)
                system.add_init_feature(function(particle) {
                    particle.colorvel = [data.color_velocity[0],
                                         data.color_velocity[1],
            ...
            if (data.color_velocity_variation) {
                var colorvelv = _round_rgba(data.color_velocity_variation);
                system.add_move_feature(function(particle) {
                    particle.color[0] += _rand_num(colorvelv[0]);
                    particle.color[1] += _rand_num(colorvelv[1]);
                    particle.color[2] += _rand_num(colorvelv[2]);
                    particle.color[3] += _rand_num(colorvelv[3]);
                });
            }
            ...
            if (data.color_velocity)
                system.add_move_feature(function(particle) {
                    ...
                    particle.color[2] += particle.colorvel[2];
                    particle.color[3] += particle.colorvel[3];
                });
            ...
            if (data.stroke_width)
                system.add_init_feature(function(particle) {
                    particle.strw = data.stroke_width;
                });

            // stroke width variation
            if (data.stroke_width_variation) {
                var strwv = _round_vec_2(data.stroke_width_variation);
                system.add_init_feature(function(particle) {
                    particle.strw += _rand_num(strwv);
                });
            }
            ...
            if (data.stroke_width) {
                system.add_draw_feature(function(particle){
                    ...
                    ctx.strokeStyle = 'rgba('+ tmp_color[0] + ','
                                             ...
                                             + tmp_color[2] + ','
                                             + tmp_color[3] + ')';
                    //console.log(JSON.stringify(ctx.strokeStyle));//junk
                    ctx.stroke();
                });
            }
        }//end if shape
...
////////////////////////////////////////////
///   PARTICLE SYSTEM BUILDER   ////////////    // (REDOING ABOVE)
////////////////////////////////////////////
var ParticleSystemBuilder = {
    ...
    create_custom : function(sys, data) {
        ...
        sys.context = sys.buffer.getContext('2d');
        data.particle_data.context = sys.context;
        sys.scene = data.scene;
        sys.particle_data = data.particle_data;
        ...
        // transformation related properties
        sys.pos   = data.position;
        sys.ppos  = data.particle_position;
        sys.rot   = data.rotation * Math.PI / 180;
        sys.scale = data.scale;
        ...
        var template_particle = new Particle(sys.particle_data, sys);
        ParticleBuilder.create_features(template_particle, sys, sys.particle_data);
    },
